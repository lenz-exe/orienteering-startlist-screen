...
def validate_iof_xml_3_0(xml_file_path: str, xsd_file_path: str) -> bool:
    try:
        ...
        if is_valid:
            logger.info(f"XML file '{xml_file_path}' is valid against XSD '{xsd_file_path}'")
        else:
            logger.info(f"XML file '{xml_file_path}' is not valid against XSD '{xsd_file_path}'")
        return is_valid
...
def pars_participants_iof_xml_3_0(xml_file_path: str, xsd_file_path: str) -> dict[str, dict[datetime, List[Participant]]]:
    try:
        validate_result = validate_iof_xml_3_0(xml_file_path, xsd_file_path)
        if not validate_result:
            raise Exception(f"XML file '{xml_file_path}' is not valid against XSD '{xsd_file_path}'")
    except Exception as e:
        logger.error(f"Parsing IOF XML 3.0 failed. XML: {xml_file_path} | XSD: {xsd_file_path}")
        logger.error(str(e))
        raise Exception(f"Parsing IOF XML 3.0 failed. XML: {xml_file_path} | XSD: {xsd_file_path}\n Exception: {e}")
    random_start_name = f"undefined_start_name_{uuid.uuid4()}"
    try:
        # Load the schema
        schema = xmlschema.XMLSchema(xsd_file_path)
        # Parse the XML file into a dictionary
        xml_as_dict = schema.to_dict(xml_file_path)
        start_list: dict[str, dict[datetime, List[Participant]]] = {}
        for index, class_start in enumerate(xml_as_dict.get("ClassStart", []), start=1):
            ...
            for person in class_start.get("PersonStart", []):
                ...
                country_short = person.get('Organisation', {}).get('Country', {}).get('@code')
                tmp_start_time = person.get('Start', [])[0].get('StartTime', '2000-01-01T00:00:00+00:00')
